...
namespace NVIDIA.Flex
{
    ...
    public class PointOctreeNode<T>
    {
        ...
        //Minimum size of the node in this octree;
        float minSize;
        ...
        // A generally good number seems to be around 8 - 15
        const int NUM_OBJECTS_ALLOWED = 8;

        //For reverting the bounds size ater temporary changes
        ...
        /// <returns>true when the object is added</returns>
        ...
        /// <param name="obj">Object to remove</param>
        /// <param name="objPos">Position of the object</param>
        /// <returns>True if the object was remove successfully</returns>
        ...
        ///<summary>
        ///Return objects are within maxDistance of the specified ray.
        /// </summary>
        ...
        {
            //Does ray hit this node at all?
            //Note: Expanding the bounds is not exadctly the same as real distance check, but its fast.
            //TODO: need to have fast AND accurate formula to do this check
        ...
        ///- this node is >= minLength in length
        ///-All objects in the root node are within one octant
        ///-this node doesnt have children, or does but 7/8 children are empty
        ///we can also shrink it if there are no object left at all!
        /// </summary>
        /// <param name="minLength">Minimum dimensions of a node in this octree</param>
        /// <returns>the new root, or the existing one if we didnt shrink</returns>
        public PointOctreeNode<T> ShrinkIfPossible(float minLength)
        {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                ...
                {
                    return this;//cant reduce - objects fit in different octants
                }
            }
            ...
            if (children != null)
            {
                ...
                for (int i = 0; i < children.Length; i++)
                {
                    if (children[i].HasAnyObjects())
                    {
                        if (childHadContent)
                        {
                            return this; //cant shrink- another child has content already.
                        }
            ...
            if (children == null)
            {
                //We dont have any children, so just shrink this node to the new size
        ...
        /// </summary>
        /// <param name="baseLengthVal">Length of this node, not taking looseness into account</param>
        /// <param name="minSizeVal">Minimum soze of nodes in this octree</param>
        /// <param name="centerVal">Centre position of this node</param>
        ...
        void SubAdd(T obj, Vector3 objPos)
        {
            ...
            //so we can skip checks and simply mve down if there are children already
            if (!HasChildren)
            {
                ...
                if (children == null)
                {
                    ...
                    //Now that we have new children, move this node's existing objects into them
                    for (int i = objects.Count - 1; i >= 0; i--)
                    {
                        OctreeObject existingObj = objects[i];
                        //Find which child the object is closest to based on where
                        //the objects center is located in relation to the octee's centre
